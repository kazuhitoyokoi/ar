...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <html>
      ...
      window.updateWsStatus = function(isOpen) {
        ...
        if (dot) {
          ...
          <xsl:if test="//SWITCH | //LINK | //STREAMER | //TEXT/LINK | //VIEWER/LINK">
            ...
            <a-assets timeout="10000">
              <xsl:for-each select="//STREAMER">
                <xsl:variable name="fbLang">
                  <xsl:choose><xsl:when test="METADATA/@fallback"><xsl:value-of select="METADATA/@fallback"/></xsl:when><xsl:otherwise>en</xsl:otherwise></xsl:choose>
                </xsl:variable>
                <xsl:variable name="vUrl">
                  <xsl:choose>
                    <xsl:when test="@url != ''"><xsl:value-of select="@url"/></xsl:when>
                    <xsl:when test="METADATA/url/*[local-name()=$fbLang] != ''"><xsl:value-of select="METADATA/url/*[local-name()=$fbLang]"/></xsl:when>
                    <xsl:when test="METADATA/url/*[1] != ''"><xsl:value-of select="METADATA/url/*[1]"/></xsl:when>
                  </xsl:choose>
                </xsl:variable>
                <xsl:if test="$vUrl != ''">
  ...
  <xsl:template match="LINK">
    ...
    <xsl:variable name="linkUrl">
      <xsl:choose>
        <xsl:when test="@refer != ''"><xsl:value-of select="@refer"/></xsl:when>
        <xsl:when test="METADATA/refer/*[local-name()=$fbLang] != ''"><xsl:value-of select="METADATA/refer/*[local-name()=$fbLang]"/></xsl:when>
        <xsl:when test="METADATA/refer/*[1] != ''"><xsl:value-of select="METADATA/refer/*[1]"/></xsl:when>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="MODEL">
    <a-entity>
      ...
      <xsl:if test="LINK">
        <xsl:variable name="fbLang">
          <xsl:choose><xsl:when test="LINK/METADATA/@fallback"><xsl:value-of select="LINK/METADATA/@fallback"/></xsl:when><xsl:otherwise>en</xsl:otherwise></xsl:choose>
        </xsl:variable>
        <xsl:variable name="linkUrl">
          <xsl:choose>
            <xsl:when test="LINK/@refer != ''"><xsl:value-of select="LINK/@refer"/></xsl:when>
            <xsl:when test="LINK/METADATA/refer/*[local-name()=$fbLang] != ''"><xsl:value-of select="LINK/METADATA/refer/*[local-name()=$fbLang]"/></xsl:when>
            <xsl:when test="LINK/METADATA/refer/*[1] != ''"><xsl:value-of select="LINK/METADATA/refer/*[1]"/></xsl:when>
          </xsl:choose>
        </xsl:variable>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:choose>
            <xsl:when test="ancestor::TARGET or ancestor::IMGTARGET"><xsl:attribute name="scale">1 1 1</xsl:attribute></xsl:when>
  ...
  <xsl:template match="VIEWER">
    ...
    <xsl:variable name="picUrl">
      <xsl:choose>
        <xsl:when test="@picture != ''"><xsl:value-of select="@picture"/></xsl:when>
        <xsl:when test="METADATA/picture/*[local-name()=$fbLang] != ''"><xsl:value-of select="METADATA/picture/*[local-name()=$fbLang]"/></xsl:when>
        <xsl:when test="METADATA/picture/*[1] != ''"><xsl:value-of select="METADATA/picture/*[1]"/></xsl:when>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="TEXT | COUNTER | VUMETER">
    ...
    <xsl:choose>
      <xsl:when test="LINK">
        <xsl:variable name="fbLang"><xsl:choose><xsl:when test="LINK/METADATA/@fallback"><xsl:value-of select="LINK/METADATA/@fallback"/></xsl:when><xsl:otherwise>en</xsl:otherwise></xsl:choose></xsl:variable>
        <xsl:variable name="linkUrl"><xsl:choose>
          <xsl:when test="LINK/@refer != ''"><xsl:value-of select="LINK/@refer"/></xsl:when>
          <xsl:when test="LINK/METADATA/refer/*[local-name()=$fbLang] != ''"><xsl:value-of select="LINK/METADATA/refer/*[local-name()=$fbLang]"/></xsl:when>
          <xsl:when test="LINK/METADATA/refer/*[1] != ''"><xsl:value-of select="LINK/METADATA/refer/*[1]"/></xsl:when>
        </xsl:choose></xsl:variable>
      ...
      <xsl:otherwise>
        <a-text align="center" color="white" outlineColor="black" outlineWidth="0.1" side="double">
          ...
          <xsl:if test="@rgba or @backrgba">
            <xsl:attribute name="fdar-color">
              <xsl:if test="@rgba">rgba: <xsl:value-of select="@rgba"/>; </xsl:if>
              <xsl:if test="@backrgba">backrgba: <xsl:value-of select="@backrgba"/>; </xsl:if>
            </xsl:attribute>
          </xsl:if>
          <xsl:attribute name="scale"><xsl:value-of select="$textScale"/></xsl:attribute>
          <xsl:choose>
            <xsl:when test="starts-with($txtVal, '@anim:')">
              <xsl:attribute name="ws-value-updater">key: <xsl:value-of select="substring-after($txtVal, '@anim:')"/>; url: <xsl:value-of select="$wsUrl"/></xsl:attribute>
              <xsl:attribute name="value">---</xsl:attribute>
